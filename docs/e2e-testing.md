...
| T53: Drop dir mounted at its own path, unsafe values refused | With `SAGENT_DROP_DIR` the wrapper mounts the folder read-write at the same path (trailing slash dropped, none without the setting); a relative path, a missing directory (also with `~/`), `/` and the workspace itself are refused with a reason; a real `shell` run reads a host file there and leaves one behind | -- |
