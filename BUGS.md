...
| 74 | On Rancher Desktop a workspace outside the home directory silently mounted as an empty directory (its VM shares only `$HOME` and `/tmp/rancher-desktop`), so the agent saw no files and no error | `validate_workspace` recognizes Rancher Desktop by its `rancher-desktop` docker context and refuses such a workspace with a message naming the shared paths; `SAGENT_SKIP_SHARE_CHECK=1` covers setups that added shares via lima's override.yaml; T33 |
